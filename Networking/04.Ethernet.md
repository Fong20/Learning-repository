...
- Usually when we transfer data, we don't know the destination MAC address but we know the destination IP address because of the DNS. Thus, **we use ARP to find the destination MAC address that is mapped to the destination IPV4 address.** It does so by sending the ARP request (destination IP address) to all devices to check if destination IP address is the same. If it is the same, it will send back the information of the destination of the MAC address to the source end device. The source end device then adds the destination MAC address and IP address into the ARP table.
...
3. Because the destination address is a broadcast type of communication, the switch forwards the frame to all ports (PC2 & PC3) except for the port which received the frame (PC1).
4. The destination device replies to the broadcasted frame with a unicast frame addressed to PC 1.
5. Before the unicast frame can reach PC1,the switch enters the MAC address and the port number of the switch port of the destination device into the MAC address table. The destination MAC address and its associated port can now be found in the MAC address table as well.
...
### Store and forward
In store and forward method, it will check the received frame and check the CRC. If the CRC is valid, it will then look into the destination MAC address and send the frame to the destination through the correct port.
...
![image](https://github.com/Fong20/Learning-repository/assets/150316121/4601a9b1-e924-4034-b057-59b8643c5b4a)
  ...
  Fast forward switching has the lowest level of latency as it immediately forwards the packet to the destination after reading the destination address.

  ### Fragment-free switching
  The switch stores the first 64 bytes of the frame before forwarding, most network errors and collisions occur at the first 64 bytes.
