...
## Structural dependence vs Structural Independence (file structure)
  ...
  - Structural independence access to a file is not dependent on its own structure.
...
- data storage characteristics is changed without affecting the program’s ability to access the data.
  ...
  - **Logical data independence (conceptual schema)** = the program which access the table does not need to change when the file structure is changed. Eg: addnew columns, remove new columns from table etc
  - **Physical data independence (physical schema)** =  the program which access the table does not need to change when the data structure is changed. Eg: change/upgrade storage system
...
- **Islands of information**: Scattered data locations. This increases the probability of having different versions ofthe same data
