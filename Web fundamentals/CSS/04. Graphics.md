...
- To create an image hyperlink, we need to surround the image element with with anchor tags. By doing this, we are replacing the text that normally sits in between the anchor tags to an image.
  ...
  ### Image map
  - Image map applies multiple clickable or selectable hyperlinks to be implemented in different areas of the image.
  ...
  - In short, image map allows us to insert multiple hyperlinks in a single image by setting up the hyperlinks in certain areas of the image.
