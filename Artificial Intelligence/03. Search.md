...
How it works
- It works by choosing the nodes at each of the layer with the lowest straight line distance to the goal and expanding on it
...
- admissible heuristic – never overestimates the cost to reach the goal, i.e. optimistic
- i.e. the actual path cost to reach goal through node n > f(n)
- e.g. hsld(n) is always shorter than the actual cost from n to goal. Thus, hsld(n) is admissible.

![image](https://github.com/user-attachments/assets/e54ee90c-7faf-4822-9d2c-9fc366310006)

## Non-deterministic environment
- previous algorithms consider observable and deterministic environment, with the agent knowing exactly the effect of the actions
- i.e. an action on a state leads to only one state
- in a non-deterministic environment, an action applied on a state leads to multiple possible states
- the actual outcome of the action can only be identified once it’s been carried out
- percepts tell the agent which of the possible outcomes has occurred
