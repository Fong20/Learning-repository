...
### Minimax algorithm
- the rule of thumb is to increase the chance to win against the opponent
- a heuristic function is needed to measure if a state is more advantageous for one player or the other

#### Formula to calculate heuristic function
E(x) = N(x) - N(o)

N(×) as the number of rows, columns, and diagonals that × could win
N(o) as the number of that for o
E(x) = heuristic function

#### How it works
- Minimax algorithm expands the game tree up to a number of moves and evaluates the goodness of the moves based on the leaf nodes.
- Calculate E(X) for the leaf nodes
- Find the minimum value E(x) for each branch
- Move the minimum values to the max level and find the maximum value E(x) by comparing the values of each branch

![image](https://github.com/user-attachments/assets/74c90bcb-2adb-4833-88c3-1981deaefad9)

### Minimax algorithm with alpha-beta pruning
- Improves the efficiency of minimax algorithm by removing unimportant 
...
### How it works
- Beta value is found by getting the minimum value
- Alpha value is obtained by getting the minimum value of the branch
