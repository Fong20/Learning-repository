...
In slow start TCP congestion control, the number of segments are transmiited exponentially until certain criterias are met.

**Improtant concepts:**
- cwnd = ssthresh (threshold limit)
- cwnd value = 1 Maximum Segment Size (MSS, maximum amount of data which can be included in a segment)

**How it works?**
- When connection begins, It will start by sending 1 segment and if it receives a successful acknowledgement, the cwnd will be doubled for every RTT until the first packet loss occurs or the cwnd threshold is reached
- Thus, the intial rate is slow but it increases exponentially
  
![image](https://github.com/user-attachments/assets/b2502d63-f18b-4b5c-83d7-3a020430d3aa)

### TCP Congestion control: Congestion avoidance
- 1 RTT = 1 ACK received
- When connection cwnd = ssthresh, Increase cwnd by 1 MSS for every ACK received
- When packet loss occurs, the cwnd / 2 and set cwnd = 1 MSS

### TCP Congestion control: Packet loss
Two congestion control approaches are used to deal with packet loss which could occur due to timeout or triple duplicate ACK received as seen below:
...
3. **TCP Reno (aggressive approach)**
   - When packet loss occurs regardless due to timeout or triple duplicate ACK received, it will set ssthresh = cwnd / 2
