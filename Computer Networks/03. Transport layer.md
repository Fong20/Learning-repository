...
However, it will stop increasing the congestion window exponentially once the first packet loss occurs or the cwnd threshold is reached.
...
- When the congestion window reaches the slow start threshold (ssthresh), congestion window is increased by 1 for every ACK received (cwnd = cwnd + 1). This is known as the additive increment process
- When packet loss occurs, the slow start threshold (ssthresh) is set to congestion window is divided by 2 (cwnd / 2) and set cwnd = 1 
