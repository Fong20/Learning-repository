...
Multiplicative decrement occurs in this phase where congestion window is reduced when there is congestion. The only way a sender can guess that congestion has happened is though the occurence of packet loss where there is the need to retransmit a segment. Retransmission is needed to recover a missing packet that is assumed to have been dropped by a router due to congestion. Packet loss can occur in either of the two cases which are:
