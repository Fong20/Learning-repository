...
1. co-channel = Co-channel interference occurs when multiple devices is operating on the same frequency. It hinders performance by increasing the wait time as the same channel is used by different devices.
...
### Frequency division multiplexing (FDM)
- FDM divides the overall bandwidth available in a communication channel into a number of non-overlapping frequency bands, each of which carries a distinct signal. These independent signals share a single transmission channel, such as a cable or optical fiber. A dedicated frequency band is assigned to each connection. The width of the frequency band indicates the bandwidth. Frequency is divided between different users
...
- In TDM, the frequency remains the same, but the time in which it is used differs
...
**In circuit switching, the resource must be reserved (dedicated end-to-end connection or circuit) from the source host to the destination host for the entire duration of the communication session.** The allocated resources may not be fully utilized or maybe insufficient which cannot be used to transfer other packets.
