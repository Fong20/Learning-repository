...
- A process can migrate among various queues throughout its lifetime, scheduling of process is done through different schedulers.
  ...
  ![image](https://github.com/Fong20/Learning-repository/assets/150316121/dfeaa186-eb97-4182-8e52-51d9cdca9c37)
    ...
    **CPU scheduling decisions may take place when a process**
    ...
    ### Long-term scheduler
    - Select a process to be brought to ready queue
...
## Support for threads (thread design model)
  ...
  ### Pure user level threads (Many to one model)
  - Many user threads are mapped to a kernel thread
  - **The user threads are created and managed by the user-level thread library.**
  - Although there are multiple threads, they are unable to run in parallel. Only one thread can access the kernel at a time by making a system call. Once a thread makes a system call to access the kernel thread, other threads in the same process cannot access the kernel thread.
  ...
  ### Pure kernel level threads (one to one model)
  - In pure kernel level thread, each user thread is mapped to a kernel thread. Thus, each thread can be allocated to each processor run parallel on mutiprocessors
...
## Thread pools
Thread pools are threads which are pre-created for faster access
...
2. Message Passing (call function to the OS and the OS transfers the function to another process)
  ...
  ### Shared memory 
  - Shared memory us also known as a buffer
  - Process A shares memory with Process B to communciate with each other. Process A shares the data to the shared memory whereas Process B consumes the data from the shared memory
  - Producer = shares the data
  - Consumer = Consumes the data

    Synchronization error/communication fail occurs when the process tries to acquire the data from the shared memory but there is no data in the shared memory
    
  ![image](https://github.com/Fong20/Learning-repository/assets/150316121/1e0f8a7a-1cc4-48ef-a5da-d1adc8ef150d)

  ### Race condition
  Race condition is a condition which occurs in shared memory communication whereby when two processes are trying to access the same data/resource at the same time

  **Example **
  ![image](https://github.com/Fong20/Learning-repository/assets/150316121/f7fa9f74-c749-4202-93bd-19ae2f0de0d0)

  The solution to solving Race Condition is by locking the variable. By locking the variable, it prevents processes from accessing the resources until the particular process is finished executed.
  ...
  **Type of communication link**
  ...
  - In an indirect communication, communication between processes go through a mailbox.
  - Indirect communication is used in morern OS
  ...
  - Synchronous means the sender cannot send another messsage until the message is received by the receiving process.
