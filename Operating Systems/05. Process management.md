...
There are two main threading models in process management: user-level threads and kernel-level threads.
  ...
  - **In the user level thread model, the operating system does not directly manage the threads.** Instead, **threads are managed by a user-level thread library, which is part of the application.** Thus, switching between threads does not interrupt the kernel
  - Many user threads are mapped to a kernel thread
  - Although there are multiple threads, they are unable to run parallaly as only one thread can access the kernel at a time by making a system call. Once a thread makes a system call to access the kernel thread, other threads in the same process cannot access the kernel thread.
  ...
  **Disadvantages**
  1. The whole process is blocked if one of the user-level thread runs a blocking operation.
...
![image](https://github.com/Fong20/Learning-repository/assets/150316121/aa20dc51-e263-4dcb-a27d-b1da9c2954bb)
  ...
  - If any thread makes a blocking system call, the kernel can still schedule another thread for execution.
...
## Process deadlock
- Process deadlock is said to occur when one of the process is unable to obtain the resources to run.
- It reduces system performance as resources are held by processes which cannot be executed. As a result, this prevents other processes from being executed as well.
- Eventually, the system will stop functioning and will need to be restarted.

**General rule of process and resources**
1. Process request to use the resources and it may request more than one resource to carry out its designated task. However, it cannot request more than the amount of available resources.
2. After the process finishes its execution, the resources are released.
3. If the process request for resources and the resources are not available, then the process will enter the waiting state.

![image](https://github.com/Fong20/Learning-repository/assets/150316121/a3718adb-4283-49ef-9443-dc31a11dab6a)

All the usage of resources or free resources will be recorded in the System table

A waiting process can never exit the waiting state as the resources it has requested are held by another waiting process. This will eventually cause deadlock
...
![image](https://github.com/Fong20/Learning-repository/assets/150316121/ec6e752f-95a3-4f6a-9157-ec5d7ff4c102)

  ### Mutual exclusion
  Mutual exclusion means no two or more processes can request access to a non sharable resource, only one process can have access to the resource. This will eventually cause deadlock

  ### Hold and wait
  In hold and wait, a process holds at least one resource and waits to acquire additional resources that are being held by other resources.
  
  Example: Both the scanner and dvd processes are in the waiting state and cannot obtain the resources from one another as both of the processes are holding their respective resources. This causes deadlock
  ![image](https://github.com/Fong20/Learning-repository/assets/150316121/cd5f9b7a-ac16-4bb4-8bf5-c0c74148c116)

  ### No preemption
  **The process which is scheduled will be executed and utilize the resources till its completion.** **The resources can only be released by the process which is currently being executed. No other processes can be scheduled by the scheduler.**

  ### Circular wait
  - Circular wait is an extension of hold and wait
  - **All the processes must be in the waiting for the resources in a cyclic manner** so that **the last process is waiting for the resource which is being held by the first process**
...
### Resource allocation graph
Resource allocation graph is a useful tool which is used to determine if there is any process deadlock.

**Components of a resource allocating graph**
1. Vertices = used to represent processes and resources
2. Edges = arrows

![image](https://github.com/Fong20/Learning-repository/assets/150316121/df7973c9-e8c1-41ea-a407-7d9931a8d287)

**Example 1: with deadlock**
![image](https://github.com/Fong20/Learning-repository/assets/150316121/68c2c1d5-719d-4ac6-953c-63b4d3f31f87)

![image](https://github.com/Fong20/Learning-repository/assets/150316121/5ade9e79-fb7a-4e7b-9b67-9d2da8a1992a)


**Example 2: without deadlock**
![image](https://github.com/Fong20/Learning-repository/assets/150316121/0204a581-b01d-45f3-a31d-f7e660a18d1a)

![image](https://github.com/Fong20/Learning-repository/assets/150316121/a732278b-5f7d-46d4-a203-8e0dcdf24ffe)

### Methods to handle deadlock
- deadlock prevention (mechanism to handle deadlock)
- deadlock avoidance (mechanism to handle deadlock)
- deadlock detection and recovery (Deadlock already occurs but leave it to the OS to handle the deadlock)
- ignore the problem and restart the computer

  ### Deadlock prevention
  - As stated above, deadlock occurs from the four conditons. The **basic idea of deadlock prevention is to violate any one of the four conditions. (prevent either one of these conditions from happening)**
  - only condition 4 is possible whereas condition 1, 2 and 3 are not possible

  **Condition 1: Mutual exclusion**
  - solution: Allow access to resources which can be shared (Eg: read-only files)
  - However, nothing can be done if the resources are non-sharable

  **Condition 2: Hold and wait**
  - There are two protocols to solve hold and wait
  - **Protocol 1: Allocate all or almost all of the requested resources to the process before it starts**
  - **Protocol 2: Allow a process to request for resources only when it has none. Before a process can request for additional resources, it must release all the resources that it is currently holding.**
  - **The drawback of both protocols is that the process might not be able to start if at least one of the resources which is required is already allocated to other processes.**

  **Condition 3: No preemption**
  - There are two protocols to solve no preemption
  - Protocol 1: If a process is holding some resources and requests for additional resources which cannot be immediately allocated, the resources which are being holded by the process will be preempted (released) to prevent hold and wait. The process will only be restarted when it manages to obtain all of the resources it need (initial resource + resource which needs to be obtained)
  - Protocol 2: A process requests for additional resources and checks if it is currently allocated to a running process. If the ressource is being allocated to a running process, then put the running process into waiting state and force release the resources. The resource can then be used by the requesting process.

  **Condition 4: Circular wait**
  - In circular wait, each type of resources is assigned an integer number (rank number)
  - Resources of the same type have the same rank number whereas resources of different type get distinct rank numbers
  - How it works is basically processes will request resources in a strictly increasing order of rank number to avoid the forming of a loop

  ![image](https://github.com/Fong20/Learning-repository/assets/150316121/87ac9ab4-6feb-47dd-9dd2-c6e98915a961)

  ![image](https://github.com/Fong20/Learning-repository/assets/150316121/58281630-8cab-4023-ab80-d4a29a8b2587)

  ![image](https://github.com/Fong20/Learning-repository/assets/150316121/8272049a-839b-498a-a38d-50cc7608dfac)

  ### Deadlock avoidance
  Main purpose is to avoid unsafe allocation of resource. The operating sytem, additional information of the resources that a process will request and use during its lifetime is given in advance.

  **How it works?**
  - The operating system needs to continuously examine the resource allocation in order to avoid unsafe allocation of resources. This requires each process to declare the maximum number of resources that it may require at the beginning
  - When a proces requests for resources, the system may use certain algorithms (Eg: Banker's algorithm) to test if the allocation may lead to a deadlock situation
  - It also uses the resource allocation graph but with an additional type of edge (claim-edge = request for resource in the future)

  ![image](https://github.com/Fong20/Learning-repository/assets/150316121/1726c04c-70c1-436d-9de5-5d209083dace)

  ![image](https://github.com/Fong20/Learning-repository/assets/150316121/d36a41d2-a3c3-4e75-9020-b765663e2b35)


  ### Deadlock detection and recovery
  - In Deadlock detection and recovery, the **system provides an algorithm which examines if the system has encountered deadlock and attempts to recover from a deadock**
  - **It uses a variant of the resource allocation graph called the wait-for graph to detect for any deadlock** **The graph is obtained by removing the resource nodes and collapsing the appropriate edges**

  Methods of recovering from a deadlock
  1. Process termination
  2. Resource preemption

  ### Process termination
  In process termination, there are two options:

  Option 1: abort all deadlock processes
  
  Option 2: Abort one process at a time until the deadlock cycle is eliminated. After each process is aborted, a deadlock detection algorithmis used to determine whether any processes are still in deadlock. However, this option will produce more overhead

  ### Process preemption
  In resource preemption, some resources from a process is preempted and given to other processes until the deadlock cycle is broken.
