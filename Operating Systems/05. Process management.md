...
In multilevel queue scheduling, the ready queue is further partioned into separate queues and each queue can have its own scheduling algorithm based on its importance and urgency of execution

Components of a multilevel queue scheduling
1. system queue = system processes. It has the most foreground and shirter burst time
2. foreground = Consists of processes which are urgent and needed to be executed immediately
3. background = has the lower priority. It has a longer burst time
...
1. Asymmetric multiprocessing (multiple cpu cores share the samesystem resources)
2. Symmetric multiprocessing

  ## Load balancing
  Load balancing is performed to ensure that the workload is balanced among all processors to fully utilize the benefits of a multiprocessor.
...
- Traditional process has only a single thread, so one process can perform only one task at a time. If a process has multiple threads, it can perform more than one task at a time.

The Basic unit of a process with a single thread is the process whereas the basic unit of a process with multiple thread is the thread.

![image](https://github.com/Fong20/Learning-repository/assets/150316121/8c0d8fb5-b4b6-4cf6-b3d7-e60d1eee133d)

As process has its own PCB, a thread also has its own TCB

One of the benefits of threading is that it saves memeory space
