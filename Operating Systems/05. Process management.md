...
- round robin (RR)
  ...
  **Example 3: Processes with different arrival time**
  ...
  ### Shortest Job First (SJF)
  - Associate each process to bee xecuted first based on its burst time
  ...
  If there are different arrival times for each process, we need to schedule the process based on the arrival time for the first process. Once the first process is completed, the subsequent next process is then selected based on the shortest burst time.
  ...
  - Preemtive = A running processs can be interrupted by another process with higher priority
    ...
    **Example 1: Non-preemptive with same arrival time**
    
    If the arrival time is the same, In priority scheduling, we don't follow the arrival time but we refer to the priority number
    ...
    **Example 3: non-preemptive case with different arrival times**
    ...
    If the arrival time is different for each process, we need to refer to the arrival time of the process. Once the first process is finished executed, the next process is allocated to the CPU based on the priority number.
